...
import { Wallet, X, RefreshCw, ShieldCheck, Zap, Headset, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function Premium({ onClose }: PremiumProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-surface flex flex-col overflow-y-auto no-scrollbar">
      <header className="safe-area-header bg-transparent text-primary w-full top-0 left-0 flex items-center justify-between px-6 py-3 z-10 sticky backdrop-blur-md">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-4 pb-16 flex-1">
        ...
        <section className="grid grid-cols-2 gap-3 mb-8">
          <BenefitCard icon={<ShieldCheck size={20} />} title={t('premium.unlimitedAccounts')} desc={t('premium.unlimitedAccountsDesc')} color="text-amber-600" bgColor="bg-amber-50" />
          <BenefitCard icon={<Zap size={20} />} title={t('premium.advancedAnalytics')} desc={t('premium.advancedAnalyticsDesc')} color="text-indigo-600" bgColor="bg-indigo-50" />
          <BenefitCard icon={<RefreshCw size={20} />} title={t('premium.cloudSync')} desc={t('premium.cloudSyncDesc')} color="text-primary" bgColor="bg-primary/10" />
          <BenefitCard icon={<Headset size={20} />} title={t('premium.prioritySupport')} desc={t('premium.prioritySupportDesc')} color="text-red-600" bgColor="bg-red-50" />
        </section>
        ...
        <section className="mt-6 bg-surface-container-lowest border border-outline-variant/10 rounded-2xl p-5 space-y-4">
          ...
          {checkoutStep === 'confirm' && (
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-xs text-on-surface space-y-1">
              ...
              <p><span className="font-semibold">{t('premium.paymentMethod')}:</span> {paymentMethod}</p>
...
function BenefitCard({ icon, title, desc, color, bgColor }: { icon: React.ReactNode, title: string, desc: string, color: string, bgColor: string }) {
  return (
    <div className="bg-surface-container-lowest p-3 rounded-xl flex flex-col items-center text-center gap-2 border border-outline-variant/10">
      <div className={cn("p-1.5 rounded-lg", bgColor, color)}>
        {icon}
      </div>
      <div>
        <h3 className="font-manrope font-bold text-xs">{title}</h3>
        <p className="text-on-surface-variant text-[10px] leading-tight mt-0.5">{desc}</p>
      </div>
    </div>
  );
}
